...
def evaluate_model(model, X_train, y_train, X_test, y_test, season, week,
    ...
    if model_name is None:
        model_name = f"qb_model_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    metrics = {
        ...
        "model_name": f"qb_model_season{season}_week{week}_{datetime.now().strftime('%Y%m%d%H%M%S')}.pkl"
    }
...
def save_model(model, filename=None):
    if filename is None:
        filename = f"qb_model_{datetime.now().strftime('%Y%m%d%H%M%S')}.pkl"
...
def qb_model(season, week):
    ...
    print("Saving model...")
    save_model(model, f"qb_model_season{season}_week{week}_{datetime.now().strftime('%Y%m%d%H%M%S')}.pkl")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train QB model for a given season/week.")
    ...
    qb_model(args.season, args.week)