...
def evaluate_model(model, X_train, y_train, X_test, y_test, season, week,
    ...
    if model_name is None:
        model_name = f"te_model_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    metrics = {
        ...
        "model_name": f"te_model_season{season}_week{week}_{datetime.now().strftime('%Y%m%d%H%M%S')}.pkl"
    }
...
def save_model(model, filename=None):
    if filename is None:
        filename = f"te_model_{datetime.now().strftime('%Y%m%d%H%M%S')}.pkl"
...
def te_model(season, week):
    ...
    print("Saving model...")
    save_model(model, f"te_model_season{season}_week{week}_{datetime.now().strftime('%Y%m%d%H%M%S')}.pkl")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train te model for a given season/week.")
    ...
    te_model(args.season, args.week)